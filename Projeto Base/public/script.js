    ...
    .then(articles => {
        ...
        if (publishedArticles.length > 0) {
          ...
          if (featuredArticleImage) {
            ...
            let imageUrl = latestArticle.bannerImage;
            if (latestArticle.galleryImages && latestArticle.galleryImages.length > 0) {
              imageUrl = latestArticle.galleryImages[0]; 
            }
            featuredArticleImage.src = imageUrl;
    ...
    fetch('articles.json')
      ...
      .then(articles => {
        ...
        if (destaqueArticleContainer && allArticles.length > 0) {
          ...
          const imageUrl = (maisRecente.galleryImages && maisRecente.galleryImages.length > 0) 
                         ? maisRecente.galleryImages[0] 
                         : maisRecente.bannerImage;
          destaqueArticleContainer.innerHTML = `
            <div class="last-article">
              <div class="article-banner-image">
                <img src="${imageUrl}" alt="Banner do Artigo ${maisRecente.title}">
              </div>
...
// --- FIM AJUSTE INDEX.HTML ---
  ...
  const isSobrePageForCarousel = window.location.pathname.endsWith('sobre.html');
  if (isSobrePageForCarousel) {
    const initTeamCarousel = () => {
      const teamGrid = document.querySelector('#team .team-grid');
      const teamMembers = document.querySelectorAll('#team .team-member-card-container');
      const prevButton = document.getElementById('team-carousel-prev');
      const nextButton = document.getElementById('team-carousel-next');

      if (!teamGrid || teamMembers.length === 0 || !prevButton || !nextButton) {
      ...
      const showMember = (index) => {
        ...
        // It's the card's left offset minus half of the leftover space in the container.
        const scrollAmount = member.offsetLeft - (teamGrid.offsetWidth - member.offsetWidth) / 2;
      ...
      const startDrag = (e) => {
        isDragging = true;
        startX = (e.pageX || e.touches[0].pageX) - teamGrid.offsetLeft;
      ...
      const endDrag = () => {
        ...
        if (mediaQuery.matches) {
            ...
            const gridCenter = teamGrid.scrollLeft + teamGrid.offsetWidth / 2;
            let closestMemberIndex = -1;
            let minDistance = Infinity;

            teamMembers.forEach((member, index) => {
                const memberCenter = member.offsetLeft + member.offsetWidth / 2;
                const distance = Math.abs(gridCenter - memberCenter);
                if (distance < minDistance) {
                    minDistance = distance;
                    closestMemberIndex = index;
                }
            });

            if (closestMemberIndex !== -1) {
                showMember(closestMemberIndex);
            }
        }
      ...
      const drag = (e) => {
        ...
        e.preventDefault();
        const x = (e.pageX || e.touches[0].pageX) - teamGrid.offsetLeft;
        const walk = (x - startX) * 2; // Multiplier for faster scrolling
        teamGrid.scrollLeft = scrollLeft - walk;
      };

      // Add mouse events for desktop debugging and wider compatibility
      teamGrid.addEventListener('mousedown', startDrag);
      teamGrid.addEventListener('mouseleave', endDrag);
      teamGrid.addEventListener('mouseup', endDrag);
      teamGrid.addEventListener('mousemove', drag);

      // Add touch events for mobile
      teamGrid.addEventListener('touchstart', startDrag);
      teamGrid.addEventListener('touchend', endDrag);
      teamGrid.addEventListener('touchmove', drag);
      
      // We need to check if the screen is mobile to run the carousel logic
      const mediaQuery = window.matchMedia('(max-width: 768px)');
      
      const handleCarouselState = () => {
        if (mediaQuery.matches) {
          // On mobile, make sure the first card is centered on load/resize
          // Use a small timeout to ensure the browser has calculated the layout dimensions
          setTimeout(() => showMember(0), 100);
        }
      };

      // Listen for changes in screen size to enable/disable carousel behavior
      mediaQuery.addEventListener('change', handleCarouselState);
      
      // Initial check when the script loads to set up the carousel if on mobile
      handleCarouselState();
    };
...
// --- FIM: LÓGICA PARA ANIMAÇÃO SUAVE DOS ÍCONES NO CARD (VERSÃO CORRIGIDA) ---
  ...
  // --- START: Team Member Morphing Modal Logic (Synchronized) ---
  ...
  function openModal(memberId) {
    if (activeCard) return; // Prevent animation if one is already running

    activeCard = document.querySelector(`.team-member-card[data-member-id="${memberId}"]`);
    activeModal = document.getElementById(`modal-${memberId}`);
    if (!activeCard || !activeModal) return;

    const cardContent = activeCard.querySelectorAll('.card-front > *, .card-back > *');
    const modalContent = activeModal.querySelector('.modal-content');

    activeCard.classList.add('is-animating');

    // 1. Fade out card content
    cardContent.forEach(el => el.classList.add('is-fading-out'));
    
    // 2. After card content fades, start the morph
    cardContent[0].addEventListener('transitionend', function startMorph(event) {
      if (event.propertyName !== 'opacity') return;

      const cardRect = activeCard.getBoundingClientRect();
      const morphContainer = document.createElement('div');
      morphContainer.classList.add('morph-container');

      morphContainer.style.top = `${cardRect.top}px`;
      morphContainer.style.left = `${cardRect.left}px`;
      morphContainer.style.width = `${cardRect.width}px`;
      morphContainer.style.height = `${cardRect.height}px`;
      
      document.body.appendChild(morphContainer);
      activeCard.classList.add('is-hidden');
      activeModal.classList.add('is-open');

      // 3. Animate the container
      setTimeout(() => {
        const modalRect = modalContent.getBoundingClientRect();
        morphContainer.style.top = `${modalRect.top}px`;
        morphContainer.style.left = `${modalRect.left}px`;
        morphContainer.style.width = `${modalRect.width}px`;
        morphContainer.style.height = `${modalRect.height}px`;
      }, 10);

      // 4. After morph, fade in modal content
      morphContainer.addEventListener('transitionend', () => {
        modalContent.classList.add('is-fading-in');
        if (morphContainer.parentElement) {
          morphContainer.parentElement.removeChild(morphContainer);
        }
      }, { once: true });

      this.removeEventListener('transitionend', startMorph);
    }, { once: true });
  }
  ...
  function closeModal() {
    ...
    // 1. Fade out modal content
    modalContent.classList.remove('is-fading-in');
    modalContent.classList.add('is-fading-out');

    // 2. After content fades, start reverse morph
    modalContent.addEventListener('transitionend', function startReverseMorph(event) {
      if (event.propertyName !== 'opacity') return;
      ...
      // 3. Animate container back to card position
      setTimeout(() => {
        morphContainer.style.top = `${cardRect.top}px`;
        morphContainer.style.left = `${cardRect.left}px`;
        morphContainer.style.width = `${cardRect.width}px`;
      ...
      // 4. After reverse morph, fade in card content
      morphContainer.addEventListener('transitionend', () => {
      ...
      this.removeEventListener('transitionend', startReverseMorph);
      modalContent.classList.remove('is-fading-out');
    }, { once: true });
  }
    ...
    card.addEventListener('click', (event) => {
      ...
      if (isMobile()) {
        if (card.classList.contains('is-flipped')) {
          ...
          // Flip this card, and unflip any others
        ...
        // Desktop behavior: open modal directly
        openModal(card.dataset.memberId);
      }
    });
  });

  // Unflip cards if user clicks outside on mobile
  document.addEventListener('click', (event) => {
    if (isMobile()) {
      const clickedCardContainer = event.target.closest('.team-member-card-container');
  ...
  // --- END: Team Member Morphing Modal Logic (Synchronized) ---
