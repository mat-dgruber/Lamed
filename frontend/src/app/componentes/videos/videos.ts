...
export class Videos implements OnInit {
  ...
  videos$ = this.loadVideos(false);
  playingVideoIds = signal<Set<string>>(new Set());

  private loadVideos(isShort: boolean) {
    return this.videosService.getVideos(isShort).pipe(
      tap((videos) => {
        if (!isShort) {
          this.updateJsonLd(videos);
        }
      })
    );
  }
  ...
  setTab(tab: 'long' | 'shorts'): void {
    ...
    this.videos$ = this.loadVideos(tab === 'shorts');
  }

  refreshVideos(): void {
    this.videos$ = this.loadVideos(this.activeTab() === 'shorts');
  }
  ...
  private updateJsonLd(videos: any[]): void {
      ...
      data: {
        ...
        itemListElement: (videos || []).map((v: any, index: number) => ({
          ...
          thumbnailUrl:
            ...
            (v.id?.videoId ? `https://img.youtube.com/vi/${v.id.videoId}/hqdefault.jpg` : undefined),
